...
# One usage row of a seat, fitted to the terminal width.
ccseat__pick_usage_line() {
  local label="$1" p="$2" r="$3" dim="$4" cols="$5" pc when used
  ...
  if [ "$cols" -ge 56 ]; then
    ...
    printf '    %s%s%s  %s  %s%s%%%s  %s%s%s' "$CCSEAT_C_MID" "$label" "$CCSEAT_C_RESET" "$(ccseat_bar "$p" "$dim")" \
...
ccseat__pick_tags() {
  local i="$1" cur="$2" room="$3" compact="$4" out="" len=0 t c s sc
  ...
  if [ "${CCSEAT_ROW_AUTH[i]}" = none ]; then
    ...
    t="at its ${CCSEAT_ROW_LIMIT[i]} limit" c=$CCSEAT_C_ORANGE
  elif [ "$s" = live ]; then
    t="" c=""
  else
...
# A percentage for the one-line layout: "96%" (four wide), in orange with a
...
ccseat__pick_pct() {
  ...
  if [ "$p" -ge "$limit" ]; then
    printf '%s%s!%s' "$CCSEAT_C_ORANGE" "$(ccseat_rpad "$p%" 4)" "$CCSEAT_C_RESET"
    return
  fi
...
ccseat__pick_seat_lines() {
  local i="$1" sel="$2" cols="$3" cur="$4" name nc dim room nw
  ...
  if [ "$CCSEAT__PICK_COMPACT" = 1 ]; then
    ...
    if [ "$cols" -ge $((nw + 30)) ]; then
      printf '  %s5-hour%s %s  %sweekly%s %s' "$CCSEAT_C_MID" "$CCSEAT_C_RESET" \
        "$(ccseat__pick_pct "${CCSEAT_ROW_P5[i]}" "$dim" "$(ccseat_config_get limit_5h)")" "$CCSEAT_C_MID" "$CCSEAT_C_RESET" \
        "$(ccseat__pick_pct "${CCSEAT_ROW_P7[i]}" "$dim" "$(ccseat_config_get limit_weekly)")"
      room=$((cols - nw - 30))
    else
      printf '  %s %s' "$(ccseat__pick_pct "${CCSEAT_ROW_P5[i]}" "$dim" "$(ccseat_config_get limit_5h)")" \
        "$(ccseat__pick_pct "${CCSEAT_ROW_P7[i]}" "$dim" "$(ccseat_config_get limit_weekly)")"
      room=$((cols - nw - 15))
    fi
    printf '%s\n' "$(ccseat__pick_tags "$i" "$cur" "$room" 1)"
  ...
  printf '%s\n' "$(ccseat__pick_usage_line "5-hour" "${CCSEAT_ROW_P5[i]}" "${CCSEAT_ROW_R5[i]}" "$dim" "$cols")"
  printf '%s\n' "$(ccseat__pick_usage_line "weekly" "${CCSEAT_ROW_P7[i]}" "${CCSEAT_ROW_R7[i]}" "$dim" "$cols")"
}
...
ccseat__pick_draw() {
  local sel="$1" busy="$2" cur="$3" cols lines avail total i out title hint n
  ccseat__term_size
...
ccseat_cmd_pick() {
  local sel prev cur key rest rc moved=0 busy=0 chosen="" n fetch="" i t0 eof=0 draw
  case "${1:-}" in
  ...
  while :; do
    ...
    if [ "$rc" -ne 0 ]; then
      ...
      [ "$draw" = 1 ] && ccseat__pick_draw "$sel" "$busy" "$cur"
      continue
    fi
    prev=$sel
    case "$key" in
