...
test_bash_cs_alias() {
  two_seats_bob_current
  run_i bash --norc --noprofile -i -c 'eval "$(ccseat init bash)"; eval "cs current"'
...
test_zsh_cs_alias_and_quiet_startup() {
  command -v zsh >/dev/null 2>&1 || skip "zsh is not installed"
  two_seats_bob_current
  run_i zsh -f -i -c 'eval "$(ccseat init zsh)"; eval "cs current"'
  assert_contains "$OUT" "bob"
...
test_fish_wrapper() {
  ...
  assert_contains "$OUT" "CLAUDE_CONFIG_DIR=<unset>"
  run_i fish --no-config -i -c 'ccseat init fish | source; cs current'
  assert_contains "$OUT" "bob"
}
...
test_cs_keeps_a_function_the_user_has() {
  two_seats_bob_current
  run_i bash --norc --noprofile -i -c 'cs() { echo my-own-cs; }
eval "$(ccseat init bash)"
cs version'
  assert_contains "$OUT" "my-own-cs"
  if command -v zsh >/dev/null 2>&1; then
    run_i zsh -f -i -c 'cs() { echo my-own-cs; }
eval "$(ccseat init zsh)"
cs version'
    assert_contains "$OUT" "my-own-cs"
  fi
